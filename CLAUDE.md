  ...
  `PlannerType` is `task | plan | goal`. `EventType` (on `EventExtendedProps`) is `planner | template | travel | category | external` — the engine emits `travel`/`category` at runtime, and `external` marks read-only busy blocks imported from a connected external calendar (see "External calendars" below). The engine injects `external` blocks into its fixed-event fabric to carve slots around them, then filters both `external` and `template` back out of its assembled event list.

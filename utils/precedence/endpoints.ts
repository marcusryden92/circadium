...
// affects gating, never membership. Plans and flexibly recurring items are
// excluded: neither is a scheduling candidate the gate can bound (a plan is a
// fixed anchor, a recurring item schedules per-occurrence with no single end,
// never as its bare row), so admitting one seeds a spurious broken-gate and
// leaves the successor unbounded. Detour targets keep their own predicate
...
// above. Plans are excluded at both the endpoint and the root, and a flexibly
// recurring ROOT excludes its whole subtree (same reason as
// isValidPrecedenceEndpoint; a nested recurrence value is inert, so only the
// root's rule matters). A dangling parent chain (orphaned row) is invalid.
export function isValidDependencyEndpoint(
  byId: Map<string, Planner>,
  id: string,
): boolean {
  const row = byId.get(id);
  if (!row || row.plannerType === "plan") {
    return false;
  }
  ...
  return (
    ...
    !plannerHasFlexibleRecurrence(current) &&
    current.isTriaged
  );
}

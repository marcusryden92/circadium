  ...
  - **node-ical** for parsing subscribed ICS feeds (server-only) and the **Google Calendar API** (read-only) for connected Google accounts — the external-calendar integration (see "External calendars" in the domain model).
  ...
  - **Prefer server actions** (`"use server"` files in [actions/](actions/)) over `app/api/` routes. API routes exist only for `auth/`, `admin/`, and `integrations/google/` (the Google Calendar OAuth connect + callback handlers, which need a redirect URI a server action can't provide). (The AI assistant's old SSE route is gone — its tool-use loop runs in the browser on the user's own key; see the AI-assistant section.)
  ...
  │   ├── api/                          # auth/ + admin/ + integrations/google/ (Google Calendar OAuth connect + callback)
  ...
  │   ├── externalCalendars.ts          # External-calendar sources (ICS + Google): fetch/add/refresh/update/delete + per-event busy-exception toggle — direct actions, outside the OCC diff sync
  ...
  │   ├── useRenderEngineMessage         # Maps persisted EngineMessage rows into console cards (shared by the calendar EngineConsole + the item-detail Alerts tab; optional subtree filter)
  ...
  │   ├── migrations/                   # 0_init … add_google_calendar_connection — see "Migration history" below for the full list
  ...
  │
  ├── redux/
  │   ├── store.ts                      # { user, calendarSource, engineOutput, schedulingSettings, externalCalendar }
  │   ├── slices/
  ...
  │   ├── draft/                        # Assistant draft-domain unit tests (forest/templates/windows/precedence ops + applies + diffs + node-edge safety)
  │   ├── draggable/                    # Touch drop-target resolution (TOP_BAND/BOTTOM_BAND)
  │   ├── external-calendar/            # ICS parse, Google event mapping, ICS-URL normalization, busy-block derivation
  ...
  │   └── utils/                        # plannerBulkActions, planRecurrence, promoteSubtree, demoteRootIntoGoal, allowedTimes, taskSplitting, renderEngineMessage, travelRefreshPolicy, windowOccurrences (+ precedence/ validators)
  │
  ├── documentation/
  ...
  ```ts
  {
    ...
    recurrence?: string | null,   // plan items — JSON {freq, interval, until}; expands into occurrence events (see "Templates, plans, completed")
  ...
  `PlannerType` is `task | plan | goal | habit` (habits are flexible auto-scheduled recurring items — see "Habits" below). `EventType` (on `EventExtendedProps`) is `planner | template | travel | category | external` — the engine emits `travel`/`category` at runtime, and `external` marks read-only busy blocks imported from a connected external calendar (see "External calendars" below). The engine injects `external` blocks into its fixed-event fabric to carve slots around them, then filters both `external` and `template` back out of its assembled event list.
  ...
  **Promote / demote** move a subtree across the root boundary, as native Redux mutations only (never through the AI draft contract — a root id matching a nested canonical row is deliberately re-minted there, which would orphan precedence references):
  ...
  A user can subscribe to outside calendars so the scheduler works around commitments it doesn't own. The domain lives in [prisma/schemas/models/externalCalendar.prisma](prisma/schemas/models/externalCalendar.prisma), the pure logic in [utils/external-calendar/](utils/external-calendar/), and the server surface in [actions/externalCalendars.ts](actions/externalCalendars.ts) + [actions/googleCalendar.ts](actions/googleCalendar.ts). All of it is **outside the OCC diff sync** — direct actions that mirror results into the `externalCalendar` Redux slice (like `viewState`), so importing a calendar never bumps `dataVersion`.

  - **ExternalCalendarSource** — a per-user connected source: `kind` (`ICS | GOOGLE | MICROSOFT`; MICROSOFT is reserved/unused), `url` (feed URL for ICS, Google `calendarId` for GOOGLE), `name`, `color?`, `enabled`, `mode` (`BUSY | VISUAL`, default `BUSY`), `modeExceptions` (JSON `string[]` of provider event UIDs whose busy/visual behavior flips the source default), `lastFetchedAt?`, `lastError?` (cleared on the next successful refresh). Cascade-deleted with the user.
  ...
  - **Two connection paths.** (1) **ICS feed URL** — paste a URL in Settings → Integrations; `normalizeIcsUrl` accepts `https`/`webcal` (webcal rewritten), the server fetches it (15s timeout, 10MB cap), and `parseIcsFeed` (node-ical, server-only) expands recurrences (overrides + EXDATEs applied) into concrete occurrences. A pasted Google *sharing* link resolves its `cid` to a `calendarId` and falls back to the public `.../public/basic.ics` feed. (2) **Google OAuth** — `GET /api/integrations/google/connect` sets a CSRF state cookie and redirects to Google's consent screen (scope `calendar.readonly`, `access_type=offline`); the `callback` exchanges the code, stores the refresh token, and redirects to `/settings?google=…`. Connected users pick calendars from the API and add them as `GOOGLE`-kind sources.
  - **BUSY vs VISUAL.** A `BUSY` source's events block the scheduler; a `VISUAL` source is overlay-only. `modeExceptions` flips individual events against the source default (the calendar popover's "Blocks scheduling" switch toggles one UID via `toggleExternalEventBusyException`). [deriveExternalBusyEvents](utils/external-calendar/deriveExternalBusyEvents.ts) resolves which imported events actually block (`busyByDefault !== excepted`, timed only, end > start) into `SimpleEvent`-shaped blocks stamped `eventType: external`. These are **derived fresh per engine run** (in the thunk + `useManuallyRefreshCalendar`), passed as `options.externalBusyEvents`, joined onto the engine's fixed-event fabric before slot building so slots carve around them, and filtered back out of the persisted output at final assembly — so a mode/exception/enabled change takes effect on the next regen with no reload. The engine never persists them; they live only in `ExternalEvent` rows owned by the refresh path.
  - **Freshness + rendering.** [refreshPolicy.ts](utils/external-calendar/refreshPolicy.ts) sets the TTL (1h) and expansion window (30d past / 180d future); CalendarProvider's once-per-load bootstrap hydrates the slice then refreshes each TTL-stale source before one regen. On the main calendar, imported events render as a **read-only overlay** via `externalEventsToEventInput` + [ExternalEventContent](components/events/ExternalEventContent/ExternalEventContent.tsx)/[ExternalEventPopover](components/events/ExternalEventPopover/ExternalEventPopover.tsx) (all-day rows never render and never block in v1). Management UI is **Settings → Integrations** ([IntegrationsSection](app/(protected)/settings/_components/IntegrationsSection/IntegrationsSection.tsx)): add-by-URL, Google connect/disconnect + calendar picker, and a per-source BUSY/VISUAL toggle, color, enable, refresh, remove.

  ### Habits (dynamic recurring items)

  A **habit** is `plannerType: "habit"` — a flexible, auto-scheduled recurring item ("clean ~60min sometime Fri/Sat, weekly"; "meditate 20min every morning"). It reuses existing `Planner` columns (**zero new ones**): `recurrence` `{freq,interval,until}` = cadence, `allowedTimes` `{days,ranges}` = the placement window, `duration` = size (+ optional `splitting` `{minMinutes,maxMinutes}` reinterpreted as bounds for a **single flexible block**, never the multi-chunk split loop — `taskIsSplittable` returns false for habits). Habits default ready (`defaultReadyForType`), are never wholesale-"completed" (`plannerCompletion` early-returns like plans), and are edited on the normal item-detail page (RecurrenceSection + AllowedTimesSection + Duration now show for habit; Deadline/Splitting/DailyLimit hidden). Created via the library "New item" modal (type Habit, seeded weekly).

  - **Engine (in-pool scoring, NOT a gap-filler).** [expandHabits](utils/calendar-generation/helpers/CalendarGenerator/expandHabits.ts) enumerates each habit's occurrences over a small horizon (`HABIT_HORIZON_DAYS = 14`, so every occurrence's window is materialized on the first slot build) via the shared [habitPeriods](utils/habits/habitPeriods.ts) period model, and injects **one childless-root candidate per period** (`${habitId}|${periodKey}`) into the SAME leaf pool / scoring as tasks and goals. Each occurrence clone carries `deadline = periodEnd`, so the existing `calculateTaskUrgency` scores it like a deadline-bearing task — urgency climbs as the window closes, and whether it **places or is skipped ("missed") emerges from the same contest** (a high-priority habit can win a slot a task wanted; a low-priority one yields). The one new engine primitive is `PlannerSchedulingConstraints.placementWindowEnd` (an upper placement bound, threaded exactly like `earliestStart` through [findValidSlots](utils/calendar-generation/helpers/Scheduler/findValidSlots.ts) → [findAllFittingSlots](utils/calendar-generation/helpers/TimeSlotManager/findAllFittingSlots.ts)) — it confines a weekly occurrence to its own week so week 1 and week 2 don't both land on the nearest weekend. A `NO_SLOTS` on an already-materialized window is a **silent permanent miss** (dropped from the pool, no expansion, no engine message — occurrence `NO_SLOTS` failures are filtered, structural TOO_LARGE/IMPOSSIBLE remap+coalesce to the habit row). Occurrences are excluded from memoization ([buildMemoizedEvents](utils/calendar-generation/helpers/EventAssembler/buildMemoizedEvents.ts)) so a past uncompleted one **vanishes** while a completed one re-derives from the log. Sizing uses `flexibleBlockSizing` (single block clamped to [min,max]) in [placeLeaf](utils/calendar-generation/helpers/Scheduler/placeLeaf.ts).
  - **Completion log (out-of-band, like `viewState`/external calendars).** `HabitCompletion` rows keyed `@@unique([plannerId, occurrenceKey])` are written by direct actions ([actions/habitCompletions.ts](actions/habitCompletions.ts): log/unlog/get — **never bump `dataVersion`**), mirrored into the `habitCompletions` Redux slice, hydrated by a CalendarProvider bootstrap, and fed to the engine each run via `deriveHabitCompletions` → `options.habitCompletions`. `expandHabits` skips completed periods; [buildHabitCompletedEvents](utils/calendar-generation/helpers/EventAssembler/buildHabitCompletedEvents.ts) freezes them at their logged window (id-stable via `stabilizeEvent`). The calendar tile/popover Complete action ([EventContent](components/events/EventContent/EventContent.tsx) `onComplete` habit branch) resolves `{plannerId, occurrenceKey}` from the event id and calls `logHabitCompletion`/`unlogHabitCompletion` + dispatch + `updateAll()`.
  - **Statistics.** `/habits` ([HabitsClient](app/(protected)/habits/HabitsClient.tsx) + HabitCard) reads the planner rows + completion log and renders per-habit cadence/window, completion-rate meter, current streak, and a history strip, derived by [computeHabitStats](utils/habits/habitStats.ts) — which enumerates the SAME `habitPeriods` and classifies each completed / missed (elapsed, no row) / pending. Tests: [habit-scheduling](__tests__/calendar-generation/habit-scheduling.test.ts) (engine) + [habitStats](__tests__/utils/habitStats.test.ts).
  - **v1 limits.** The AI assistant can't author habits yet (columns survive on retained rows via spread-preserve); "N times per week" (count per period) is one-per-period for now; no min/max toggle in the create UI (the engine seam exists).
...
**Embedded mode (onboarding AI step).** `AIDraftModal` takes `embedded`/`intent`/`onSaved`/`onStateChange`/`resumeConversationId`/`onConversationIdChange` props. `intent="onboarding"` (the value `AssistantScope.intent` reserves) both threads to the engine for a prompt preamble (`intentBlock` — triage the raw brain-dump jots, which arrive as untyped tasks: shape each into a task or a goal with subtasks, set durations/deadlines, assign each to one of the user's *roles*, ready eligible goals) and tunes the instance: empty-state hint, no History popover, no most-recent `autoResume`, and an **auto-sent kickoff message** (fires once per mount after hydration AND after the resume attempt settles — `resumeSettled` from `useAIDraftState` — with items-present vs empty-forest phrasing). Instead of most-recent resume, the instance resumes **its own conversation**: the id is reported via `onConversationIdChange`, persisted in onboarding's `StoredProgress.aiConversationId`, and passed back as `resumeConversationId` so a page refresh mid-interview reopens the same chat (a missing/deleted id degrades to a fresh kickoff; unsaved tree proposals still reseed from canonical — only the chat persists). `embedded` renders inline via `embeddedRoot` (no Dialog overlay, no save/cancel banner); the host (`OnboardingAIStep`) owns the Save action, driving it through the reported `{hasChanges, isStreaming, save}` (`onStateChange`) and getting `onSaved` (not `onClose`) on save. Back out of the step confirms first when `hasChanges`; a confirmed discard also clears the stored conversation id, so returning starts a fresh interview.
  ...
  | habits | `/habits` | Habit tracker — cadence/window, completion-rate, streak, history per habit |
  ...
  **No Tailwind.** The leftover `components.json` is dormant — ignore it. Co-locate styles next to the component: `Foo/Foo.tsx` + `Foo/Foo.css.ts`. The Vanilla Extract plugin is wired in `next.config.mjs`.
  ...
  - `add_habit` — adds `habit` to the `PlannerType` enum + `HabitCompletion` (one row per completed habit occurrence, `@@unique([plannerId, occurrenceKey])`, cascade on planner + user). Habits are flexible auto-scheduled recurring items reusing existing `Planner` columns (recurrence/allowedTimes/duration/splitting); completions are logged OUTSIDE the OCC diff sync (never bump `dataVersion`). See "Habits" in the domain model.
  ...
  - **external-busy-blocks** — imported external-calendar events resolve (through the real `deriveExternalBusyEvents` + mode/exception resolution) to busy blocks that carve the free-slot fabric like fixed events (tasks are pushed off busy windows) while never leaking into the engine's persisted output.

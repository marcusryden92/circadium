...
// root task freezes the task occurrence, a row on a leaf under a recurring goal
// freezes that leaf's period tile. Plan check-off rows are deliberately
// skipped — the plan occurrence already renders its own anchored tile, and a
// frozen duplicate would double-book the fabric.
//
// A logged completion renders regardless of later structural edits: the log is
// the durable record, so reordering, reparenting, or giving a completed leaf
// children never hides its history (it is deliberately NOT gated on the node
// still being bottom-layer). Only two things drop it — deleting the node
// (its rows cascade away) or clearing the root's recurrence (history goes
// dormant, resurrecting if recurrence returns).

...
| `SCHEDULING_CONFIG.MAX_HORIZON_EXPANSIONS` | 30 | The maximum scheduling horizon as expansion iterations: `MAX_HORIZON_EXPANSIONS * HORIZON_CHUNK_DAYS` ≈ 2 years. The outer loop expands on demand up to this; an item that can't be reached within it (e.g. an `earliestStartDate` years out) fails loudly rather than marching the horizon over many O(fabric) passes and materializing years of background/travel events. The per-item slot finder (`findAllFittingSlots`) is called with no day cap, so it scans the whole built fabric (bounded by the placement cutoff) and never misses slots expansion has already materialized. |
| `SCHEDULING_CONFIG.UNPRODUCTIVE_EXPANSION_LIMIT` | 3 | Stop expanding after this many consecutive expansions that place nothing and are not marching the horizon toward a still-unreached `earliestStartDate` — the remaining items are constraint-stuck and more empty future room can't help them. |
...
- Stops when the pool empties (every placeable leaf scheduled). The loop expands on demand rather than at a fixed count. Two guards bound it: `UNPRODUCTIVE_EXPANSION_LIMIT` consecutive expansions that place nothing *and* are not marching toward a still-unreached `earliestStartDate` (a remaining leaf whose earliest bound is past the placeable horizon keeps expansion productive), plus `MAX_HORIZON_EXPANSIONS × HORIZON_CHUNK_DAYS` (~2 years) as the maximum horizon — an item that can't be reached within it fails loudly instead of marching indefinitely.

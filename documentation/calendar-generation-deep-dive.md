...
| `SCHEDULING_CONFIG.SLOT_SEARCH_NEAR_WINDOW_DAYS` | 90 | Preference window the per-item slot finder (`findValidSlots`) tries FIRST, measured from the task's effective earliest start. If it yields any fitting slot the task places there, so a nearer slot is always preferred over a farther one even after the fabric has expanded years out. Only when the near window is empty does the finder fall back to an uncapped scan of the whole built fabric (bounded by the placement cutoff), the reach that lets an item find slots horizon expansion has already materialized. A soft preference, never a hard cap — no slot is hidden. |

 ...
 */
/**
 * Which structural ceiling bound a TOO_LARGE failure — an emit-time fact the
 * renderer cannot reconstruct (it needs the engine's actual min, not a
 * re-derivation that could disagree). Excluded from the id, like maxCapacity:
 * an axis shift alone updates the row in place rather than resurfacing a
 * dismissed one.
 */
export type CapacityLimitingAxis =
  | "templateGaps"
  | "categoryWindow"
  | "allowedTimes"
  | "allowedTimesWindows"
...
export const CAPACITY_LIMITING_AXES: readonly CapacityLimitingAxis[] = [
  "templateGaps",
  "categoryWindow",
  "allowedTimes",
  "allowedTimesWindows",
